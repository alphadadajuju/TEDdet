...
def load_frame_detections_stream(opt, dataset, K, vlist, inference_dir):
    ...
    for iv, v in enumerate(vlist):
        ...
        # record the first K frame index (for the final frame allocate detection)
        last_k_ind = []
        ...
        for i in range(min(K  * opt_ninput, dataset._nframes[v]) - opt_ninput + 1, 1 + dataset._nframes[v]): # short-range mem: K, 1 + dataset._nframes[v] # + opt.ninput - 1
            ...
            if use_ind_flag is False and i != dataset._nframes[v]: # TODO: verify if the second condition is necessary? -> Yes, to 
                
                ind_cd -= 1
                if ind_cd == 0: # process this i frame 
                    use_ind_flag = True # TODO: don't think this is needed
                    ind_cd = opt.ninputrgb
            ...
            for label in dets:
                ...
                # to filter out low-condidence boxes; only useful for visualizing results
                ...
                for j in reversed(range(0, K)):
                    ...
                    ###
                    # reverse evaluation (in the article, we once reversed the order of testing input sequence to check whether the correct temporal structure was learned)
                    ###
                    '''
                    if vdets[last_k_ind[j]].shape[0] < 10000:
                        vdets[last_k_ind[j]] = np.concatenate((vdets[last_k_ind[j]], np.concatenate((tubelets[:, np.array([4*(inc), 1+4*(inc), 2+4*(inc), 3+4*(inc), -1])], labels), axis=1)), axis=0)
                        n_mem = n_mem - 1
                ...
                # intra-frame bounding-box extrapolation 
                ...
                for ii in range(interval):
                    ...
                    if list_update[ii] -list_update[ii + 1] > 1: # if there are gaps between
                        for d in range(len(lo_bound)): 
                            ...
                            diff_box = (hi_box - lo_box) / (abs(list_update[ii+1] - list_update[ii]))
                            
                            for iii in reversed(range(list_update[ii+1]+1, list_update[ii])):
                                if vdets[iii].shape[0] < 10000:
                                    vdets[iii] = np.concatenate((vdets[iii], np.array([np.concatenate([lo_box + (list_update[ii] - iii)*diff_box, [sc], [label-1]])])))
                ...
                for ii in range(interval):
                    ...
                    lo_bound = tubelets[:,ii*4:ii*4+4] # load from tubtlets for the current detection
                    hi_bound = tubelets[:,ii*4+4:ii*4+8]
                    scores = tubelets[:,-1]
                    assert len(lo_bound) == len(hi_bound)
    ...
    for iv, v in enumerate(vlist):
        ...
        for i in range(min(K  * opt_ninput , dataset._nframes[v]) - opt_ninput + 1  , 1 + dataset._nframes[v]): # short-range mem: K, 1 + dataset._nframes[v] # + opt.ninput - 1
            ...
            for label in dets:
                ...
                for j in range(0, K):
                    if vdets[max(i-opt_ninput*j, 1)].shape[0] <= 10000: # an arbitrarily large number to limit the max number of detection per frame
...
def BuildTubes(opt):
    ...
    for iv, v in enumerate(vlist):
        ...
        if opt.inference_mode == 'stream': # this condition may not be necessary?
            ...
            last_k_ind = []
            last_k_ind_init = min(K  * opt.ninputrgb, dataset._nframes[v]) - opt.ninputrgb + 1
            last_k_ind.append(last_k_ind_init)
            for _ in range(opt.K - 1):
                last_k_ind_init = max(1, last_k_ind_init - opt.ninputrgb)
                last_k_ind.append(last_k_ind_init)
            last_k_ind.reverse()
        ...
        for startframe in range(min(K  * opt.ninputrgb , dataset._nframes[v]) - opt.ninputrgb + 1  , 1 + dataset._nframes[v]):
            ...
            if opt.inference_mode == 'stream': # otherwise ignore
                ...
                if startframe + opt.ninputrgb >= dataset._nframes[v]:
                    k_ind_history.append(dataset._nframes[v])
                else:
                    k_ind_history.append(startframe + opt.ninputrgb)
                 
            if startframe != min(K  * opt.ninputrgb, dataset._nframes[v]) - opt.ninputrgb + 1: # not initial frame (ex: 16)
        ...
        for ilabel in range(len(dataset.labels)):
            ...
            for frame in range(min(K  * opt.ninputrgb , dataset._nframes[v]) - opt.ninputrgb + 1  , 1 + dataset._nframes[v]):
                ...
                ltubelets = VDets[frame][ilabel + 1] # Nx(4K+1) with (x1 y1 x2 y2)*K ilabel-score
                ...
                for kk in range(opt.K):
                    ...
                    #fig.suptitle(v_file[-9:], fontsize=20, color='y')
                    plt.show()
                '''
                ...
                for it, t in enumerate(CURRENT_TUBES):
                    ...
                    ious = []
                    offset =  round((frame - last_endframe) / opt.ninputrgb) # orig: frame - last_endframe
                    if offset < K: # orig: K 
                        nov = K - offset # number of overlaps
                        
                        # Slide through all overlaps 
            ...
            for t_i, t in enumerate(FINISHED_TUBES):
                ...
                # when a tubelet fails to be linked at all
                if len(t) < 2:
                    continue
                ...
                # just start new tubes
                if score_tube < 0.005: # 0.005
                    continue

                beginframe = max(t[0][0] - opt.ninputrgb*(K-1), 1)
                endframe = t[-1][0]
                ...
                for i in range(len(t)):
                    ...
                    if opt.inference_mode == 'stream':
                        ...
                        if frame != nframes: # != max number of frames
                            for k in range(K):
                                
                                out[max(frame - k*opt.ninputrgb, 1) - beginframe, 1:5] += box[4 * n_mem:4 * n_mem + 4]
                                out[max(frame - k*opt.ninputrgb, 1) - beginframe, -1] += box[-1] 
                                n_per_frame[max(frame - k*opt.ninputrgb, 1) - beginframe, 0] += 1
                                n_mem -= 1
                ...
                if 0 in out[:,-1]: # if any frame index contains 0 (meaning not filled! This line was creating issues)
                    ...
                    for nz_i, nz_v in enumerate(nonzero_ind):
                        if nz_i == len(nonzero_ind) - 1:
                            continue
                        nonzero_nonlink.append((nonzero_ind[nz_i], nonzero_ind[nz_i+1]))
...
# TODO: debug purpose; incomplete! 
def vis_linked_detection_alllabels(v, res, opt):
    # inference mode should not matter at this point (having obtained frame-wise detection)
    ...
    for ilabel in range(len(res)):
        for n_tube in res[ilabel]:
            ...
            # ignore super low-confidence tube; also consider a minimum length?
            if n_tube[1] < 0.1:
                continue
            else: 
                for n_f in n_tube[0]:
