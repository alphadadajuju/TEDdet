...
from ACT_utils.ACT_build import load_frame_detections, BuildTubes, load_frame_detections_stream
...
def frameAP(opt, print_info=True):
    ...
    if os.path.isfile(frame_detections_file) and not redo:
        ...
        if opt.inference_mode == 'stream':
            alldets = load_frame_detections_stream(opt, dataset, opt.K, vlist, inference_dirname)
        else:
    ...
    for ilabel, label in enumerate(dataset.labels): # e.g.: 0, 'brush_hair'
        ...
        # store gt video id (for evaluating untrimmed dataset)
        ...
        pr = -1*np.ones((detections.shape[0] + 1, 2), dtype=np.float32)  # precision,recall
        pr[0, 0] = 1.0
        pr[0, 1] = 0.0
        fn = sum([g.shape[0] for g in gt.values()])  # false negatives # ALPHA: == number of frames (each frame has exactly 1 action instance)
        fp = 0  # false positives
        tp = 0  # true positives
        
        for i, j in enumerate(np.argsort(-detections[:, 3])): # j: index of the det (highest to lowest score)
            k = (int(detections[j, 0]), int(detections[j, 1])) # (video id, frame id)
            ...
            # when a frame is not in the orig gt list, but its video id appears (temporal error)
            # choose to consider temporal error (untrimmed) or not (trimmed)
...
def videoAP(opt, print_info=True):
    ...
    for ilabel in range(len(dataset.labels)):
        ...
        for i, j in enumerate(np.argsort(-np.array([dd[1] for dd in detections]))): # sort by score (index 1)
        ...
        print('Failed to detect {}/{} tubes for class {}'.format(fn,num_gt,ilabel))
        res[dataset.labels[ilabel]] = pr

    # display results
    ap = 100 * np.array([pr_to_ap(res[label]) for label in dataset.labels])
    
    # display individual class performance
    frameap_percls = {}
    for cl, cls_name in enumerate(dataset.labels):
        frameap_percls[cls_name] = ap[cl]
...
if __name__ == "__main__":
    ...
    if opt.task == 'BuildTubes':
        if opt.inference_mode != 'stream':
            print('BuildTubes currently only supports inference_mode as stream (instead of normal).')
            sys.exit(0)
