...
from MOC_utils.model import create_model_rgb, load_model
...
class MOCDetector(object):
    ...
    def process(self, images, flows):
        with torch.no_grad():
            if self.rgb_model is not None and self.opt.ninput == 1:
            ...
            elif self.rgb_model is not None and self.opt.ninput > 1:
                rgb_output = self.rgb_model(flows)
            ...
            detections = moc_decode(rgb_hm, rgb_wh, rgb_mov, N=self.opt.N, K=self.opt.K, skip_mov=self.opt.skip_mov)
            return detections
    ...
    def run(self, data):
        ...
        # below code is not used for TEDdet
        if self.opt.ninput > 1:
            flows = data['flows']
