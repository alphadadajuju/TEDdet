...
from MOC_utils.model import create_model_rgb, load_coco_pretrained_model, save_model, load_model
...
# for macs computation; adust input according to needs
...
def main(opt):
    ...
    # Uncomment the block below to estimate MACs of the model
    ''' 
    # Complexity analysis (MACs)
    ...
    '''
    # Save initialized model to gurantee better reproducibility
    model_name = 'name_your_model.pth'
    epoch = 0
    save_model(os.path.join(opt.save_dir, model_name),
                       model, optimizer, epoch, 999)
    '''
    ...
    best_ap = 0
    best_epoch = 0
    stop_step = 0 # orig: 0 TODO: this needs to be adjusted otherwise lr is dropped incorrectly when resuming training! (can set to 1 now if resuming from drop-once)
    
    # added: to ensure no decrease of lr too early (for jh s1?)
    if stop_step == 0:
        drop_early_flag = False # False for more reproducible results; do not load models trained after a few number of epochs (despite good accuracy)
    else: 
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.auto_stop:
            ...
            if stop_step < len(opt.lr_step) and epoch >= opt.lr_step[stop_step]:
                ...
                # hard-coded: drops lr only if best happens with 1/2 epoch from target step (further away could just be luck?)
                if best_epoch ==  opt.lr_step[0] - 1:# or best_epoch ==  opt.lr_step[0] - 2:
                    drop_early_flag = True
                
                if drop_early_flag is False: # use the last epoch model
                    model, optimizer, _, _ = load_model(
                    model, os.path.join(opt.save_dir, 'model_last.pth'), optimizer, opt.lr)
                    drop_early_flag = True
                    print('load epoch is ', epoch)
                    
                else: # after the first drop, the rest could drop based on mAP (i.e., best)
                    model, optimizer, _, _ = load_model(
