...
class Sampler(data.Dataset):
    def __getitem__(self, id):
        ...
        # first add the cur frame
        if self.opt.use_temp_augmentation == True:
            temp_aug = random.randint(0, 1)
            
            if temp_aug == 0:
                rand_int = 0
            else:
                rand_int = self._ninputrgb // 2 #random.randint(-3, 3)
        else:
            temp_aug = random.randint(0, 0)
            rand_int = 0
            
        cur_f = np.minimum(np.maximum(frame, 1), max_frame) # otherwise: just frame
        
        # sample the first frame of the input sequence
        for _ in range(1): 
            im_inds.append(cur_f - 1)
            images.append(cv2.imread(self.imagefile(v, cur_f)).astype(np.float32))
        
        # sampled the rest of the frames 
        for _ in range(1, n_mem+1):
            
            cur_f = np.maximum(cur_f - self._ninputrgb + random.randint(-1*rand_int,rand_int), 1)
        ...
        if self.mode == 'train':
            ...
            for ilabel, tubes in self._gttubes[v].items():
                for t in tubes: # e.g., shape: (30, 5): 30 frames, frame index + coord (given ilabel class)
                    ...
                    if self._ninput == 1: # MOD: when frames may not be consecutive (rgb)
                    ...
                    else: # MOD: when frames may not be consecutive (single K + flow)
                        im_inds_true = []
